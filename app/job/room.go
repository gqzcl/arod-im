...
func (r *Room) pushproc(batch int, sigTime time.Duration) {
	var (
		...
		last time.Time
		p    *protocol.Proto
		buf  = wbyte.NewWriterSize(int(protocol.MaxBodySize))
	)
	glog.Infof("start room:%s goroutine", r.id)
	...
	for {
		...
		buf = wbyte.NewWriterSize(buf.Size())
		n = 0
		if r.c.Idle != 0 {
	...
	r.job.delRoom(r.id)
	glog.Infof("room:%s goroutine exit", r.id)
}
...
func (j *Job) getRoom(roomID string) *Room {
	...
	if !ok {
		...
		j.roomsMutex.Unlock()
		glog.Infof("new a room:%s active:%d", roomID, len(j.rooms))
	}
	return room
}
