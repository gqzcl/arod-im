...
func InitWebsocket(server *Server, addrs []string, accept int) (err error) {
	...
	for _, bind = range addrs {
		...
		glog.Infof("start ws listen: %s", bind)
		// split N core accept
		for i := 0; i < accept; i++ {
...
func serveWebsocket(s *Server, conn net.Conn, r int) {
	...
	if conf.Conf.Debug {
		...
		glog.Infof("start tcp serve \"%s\" with \"%s\"", lAddr, rAddr)
	}
...
func (s *Server) ServeWebsocket(conn net.Conn, rp, wp *bytes.Pool, tr *timer.Timer) {
	...
	trd = tr.Add(time.Duration(s.c.Protocol.HandshakeTimeout), func() {
		...
		_ = conn.Close()
		glog.Errorf("key: %s remoteIP: %s step: %d ws handshake timeout", ch.Key, conn.RemoteAddr().String(), step)
	})
	// websocket
	...
	if ws, err = websocket.Upgrade(conn, rr, wr, req); err != nil {
		...
		wp.Put(wb)
		if err != io.EOF {
			glog.Errorf("websocket.NewServerConn error(%v)", err)
		}
		return
	}
	...
	if p, err = ch.CliProto.Set(); err == nil {
		if ch.Mid, ch.Key, rid, accepts, hb, err = s.authWebsocket(ctx, ws, p, req.Header.Get("Cookie")); err == nil {
			...
			err = b.Put(rid, ch)
			if conf.Conf.Debug {
				glog.Infof("websocket connnected key:%s mid:%d proto:%+v", ch.Key, ch.Mid, p)
			}
		}
	}
	step = 4
	if err != nil {
		...
		if err != io.EOF && err != websocket.ErrMessageClose {
			glog.Errorf("key: %s remoteIP: %s step: %d ws handshake failed error(%v)", ch.Key, conn.RemoteAddr().String(), step, err)
		}
		return
	}
	trd.Key = ch.Key
	...
	for {
		...
		if p.Op == protocol.OpHeartbeat {
			...
			if conf.Conf.Debug {
				glog.Infof("websocket heartbeat receive key:%s, mid:%d", ch.Key, ch.Mid)
			}
			step++
		} else {
	...
	if err != nil && err != io.EOF && err != websocket.ErrMessageClose && !strings.Contains(err.Error(), "closed") {
		glog.Errorf("key: %s server ws failed error(%v)", ch.Key, err)
	}
	b.Del(ch)
	tr.Del(trd)
	ws.Close()
	...
	if err = s.Disconnect(ctx, ch.Mid, ch.Key); err != nil {
		glog.Errorf("key: %s operator do disconnect error(%v)", ch.Key, err)
	}
	if white {
		whitelist.Printf("key: %s disconnect error(%v)\n", ch.Key, err)
	}
	if conf.Conf.Debug {
		glog.Infof("websocket disconnected key: %s mid:%d", ch.Key, ch.Mid)
	}
}
...
func (s *Server) dispatchWebsocket(ws *websocket.Conn, wp *bytes.Pool, wb *bytes.Buffer, ch *Channel) {
	...
	if conf.Conf.Debug {
		glog.Infof("key: %s start dispatch tcp goroutine", ch.Key)
	}
	for {
		if white {
		...
		if conf.Conf.Debug {
			glog.Infof("key:%s dispatch msg:%s", ch.Key, p.Body)
		}
		switch p {
		case protocol.ProtoFinish:
			...
			if conf.Conf.Debug {
				glog.Infof("key: %s wakeup exit dispatch goroutine", ch.Key)
			}
			finish = true
			goto failed
		...
		default:
			...
			if conf.Conf.Debug {
				glog.Infof("websocket sent a message key:%s mid:%d proto:%+v", ch.Key, ch.Mid, p)
			}
		}
		if white {
...
failed:
	...
	if err != nil && err != io.EOF && err != websocket.ErrMessageClose {
		glog.Errorf("key: %s dispatch ws error(%v)", ch.Key, err)
	}
	ws.Close()
	wp.Put(wb)
	...
	if conf.Conf.Debug {
		glog.Infof("key: %s dispatch goroutine exit", ch.Key)
	}
}
