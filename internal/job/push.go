...
import (
	...
	"github.com/gqzcl/gim/api/comet"
...
func (j *Job) pushKeys(operation int32, serverID string, subKeys []string, body []byte) (err error) {
	...
	if c, ok := j.cometServers[serverID]; ok {
		if err = c.Push(&args); err != nil {
			glog.Errorf("c.Push(%v) serverID:%s error(%v)", args, serverID, err)
		}
		glog.Infof("pushKey:%s comets:%d", serverID, len(j.cometServers))
	}
	return
}
...
func (j *Job) broadcast(operation int32, body []byte, speed int32) (err error) {
	...
	for serverID, c := range comets {
		if err = c.Broadcast(&args); err != nil {
			glog.Errorf("c.Broadcast(%v) serverID:%s error(%v)", args, serverID, err)
		}
	}
	glog.Infof("broadcast comets:%d", len(comets))
	return
}
...
func (j *Job) broadcastRoomRawBytes(roomID string, body []byte) (err error) {
	...
	for serverID, c := range comets {
		if err = c.BroadcastRoom(&args); err != nil {
			glog.Errorf("c.BroadcastRoom(%v) roomID:%s serverID:%s error(%v)", args, roomID, serverID, err)
		}
	}
	glog.Infof("broadcastRoom comets:%d", len(comets))
	return
}
