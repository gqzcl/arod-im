...
func (l *Logic) PushByKeys(c context.Context, op int32, keys []string, msg []byte) (err error) {
	servers, err := l.dao.ServerByKeys(c, keys)
	if err != nil {
		return
	}
...
func (l *Logic) PushMids(c context.Context, op int32, mids []int64, msg []byte) (err error) {
	...
	for key, server := range keyServers {
		if key == "" || server == "" {
			glog.Warningf("push key:%s server:%s is empty", key, server)
			continue
		}
